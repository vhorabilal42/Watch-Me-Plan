...
<head>
<meta charset="utf-8">
<title>WatchMePlan Create Event</title>
...
<body>
	...
	<div class="container-xxl py-5 bg-dark hero-header mb-5">
		<div class="container text-center pt-5">
			<h2 class="text-white animated slideInDown">Events Pending for Approval</h2>
		</div>
	</div>
	<!-- Navbar & Hero End -->
	...
	<div class="container-xxl py-5 px-0 wow fadeInUp" data-wow-delay="0.1s">
		...
		<div class="col-md-6 bg-dark d-flex align-items-center"
			style="margin: auto; width: 1350px;">

			<div class="p-5 wow fadeInUp" data-wow-delay="0.2s">

				<!-- 	<h1 class="text-white mb-4">Events pending for approval</h1> -->
				
				<table  style="border:3px solid; width: 1200px;" cellpadding="10"  class="text-white">
				<tr>
				<th style="border:1px solid;">EventID</th>
				<th style="border:1px solid;">Event Name</th>
				<th style="border:1px solid;">Venue</th>
				<th style="border:1px solid;">Category</th>
				<th style="border:1px solid;">Type</th>
				<th style="border:1px solid;">Start Time</th>
				<th style="border:1px solid;">End Time</th>
				<th style="border:1px solid;">Price</th>
				<th style="border:1px solid;">Details</th>
				<th style="border:1px solid;">Created By</th>
				<th colspan="2" style="border:1px solid; text-align: center;">Approval</th>
				</tr>
				...
				String a = "No";
				String select = "Select * from events where isApproved=?";
				...
				while(rs.next()){
					...
					%>
						...
						<tr>
						<td style="border:1px solid;"><%=rs.getString("EventID")%></td>
						<td style="border:1px solid;"><%=rs.getString("EventName")%></td>
						<td style="border: 1px solid"><%=rs.getString("Venue")%></td>
						<td style="border:1px solid;"><%=rs.getString("Category")%></td>
						<td style="border:1px solid;"><%=rs.getString("Type")%></td>
						<td style="border:1px solid;"><%=rs.getString("StartTime")%></td>
						<td style="border:1px solid;"><%=rs.getString("EndTime")%></td>
						<td style="border:1px solid;"><%=rs.getString("Price")%></td>
						<td style="border:1px solid;"><%=rs.getString("Details")%></td>
						
						<%
						String s = "Select FirstName,LastName from user where UserID=?";
						pstmt = conn.prepareStatement(s);
						pstmt.setString(1,rs.getString("UserID"));
						ResultSet r = pstmt.executeQuery();
						r.next();
						%>
						<td style="border:1px solid;"><%=r.getString("FirstName")%>&nbsp;<%=r.getString("LastName")%></td>
						<td style="border:1px solid;"><a href='confirmEvent?uid=<%=rs.getString("UserID")%>&eid=<%=rs.getString("EventID")%>&bool=<%=yes%>'>Approve</a></td>
					 	<td style="border:1px solid;"><a href='confirmEvent?uid=<%=rs.getString("UserID")%>&eid=<%=rs.getString("EventID")%>&bool=<%=no%>'>Disapprove</a></td>
 					</tr>
			<% 	} %>
				</table>
			</div>
